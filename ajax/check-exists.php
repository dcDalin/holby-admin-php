<?php
	...
	if ( isset($_REQUEST['email']) && !empty($_REQUEST['email']) ) {
        ...
        $results = $common -> GetRows("SELECT email FROM tbl_admin WHERE email='".$email."'");
	...
	if ( isset($_REQUEST['phoneNumber']) && !empty($_REQUEST['phoneNumber']) ) {
        ...
        $phone = $common -> GetRows("SELECT phoneNumber FROM tbl_admin WHERE phoneNumber='".$phoneNumber."'");
	...
	if ( isset($_REQUEST['idNumber']) && !empty($_REQUEST['idNumber']) ) {
        ...
        $id = $common -> GetRows("SELECT idNumber FROM tbl_admin WHERE idNumber='".$idNumber."'");
