...
$('document').ready(function() {
  ...
  $("#edit-admin-form").validate({
    rules: {
      ...
      email: {
        ...
        validemail: true,
        remote: {
          url: "ajax/check-exists-edit.php",
          type: "post",
          data: {
            email: function() {
              return $("#email").val();
            },
            adminEditId: function() {
              return $('#adminEditId').val();
            }
          }
        }
      },
  ...
  function submitForm() {
    $.ajax({
        url: 'ajax/admins-edit-ajax.php',
        type: 'POST',
        data: $('#edit-admin-form').serialize(),
        dataType: 'json',
        beforeSend: function() {
          $('#btn-edit-admin').html('<img src="ajax-loader.gif" style="margin: auto; width:30px;"> &nbsp; Processing...').prop('disabled', true);
        ...
        setTimeout(function() {
          if (data.status === 'success') {
            $('#errorDiv').slideDown('fast', function() {
              $("#btn-edit-admin").html('<img src="ajax-loader.gif" style="margin: auto; width:30px;"> &nbsp; Refreshing...');
              $('#errorDiv').html('<div class="alert alert-success">' + data.message + '</div>');
              $("#edit-admin-form").trigger('reset');
              $('input[type=email],input[type=text],input[type=tel],input[type=number],input[type=password],input[type=checkbox],#gender').prop('disabled', false);
              $('#btn-edit-admin').html('Edit Admin').prop('disabled', false);
